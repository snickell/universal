...
4. Details of the HTML fragment you are to respond with:
  ...
  - embed <style> blocks hierarchically as well, inside the high-level component they will effect
    - many high level <div> should have a <style> block relevant to them and their descendants as the first element
  ...
  - if an element is intended to take input text via keyboard, please make sure it is an editable text element (e.g. <input>, <textarea>, or has contenteditable="true" set on the div, etc). DO NOT use a plain uneditable <div> for things that require text input, it will not work.
